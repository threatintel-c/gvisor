...
const (
	...
	container shareType = iota

	// pod shareType indicates that the mount is used by more than once container
	// inside the pod.
	pod
...
func (m *mountHint) parse(key, val string) error {
	switch key {
	case "src":
		m.mount.Source = val
	case "type":
		return m.parseType(val)
	case "share":
		var err error
		m.share, err = parseShare(val)
		if err != nil {
			return err
		}
	case "options":
		return m.parseOptions(val)
	default:
		return fmt.Errorf("invalid mount option: %s=%s", key, val)
	}
	return nil
}

func (m *mountHint) parseType(val string) error {
	switch val {
	case "tmpfs", "bind":
		m.mount.Type = val
	default:
		return fmt.Errorf("invalid type %type q", val)
	}
	return nil
}

func (m *mountHint) parseOptions(val string) error {
...
func newPodMountHints(spec *specs.Spec) (*podMountHints, error) {
	...
	for k, v := range spec.Annotations {
		...
		if strings.HasPrefix(k, MountPrefix) {
			...
			if err := mnt.parse(parts[4], v); err != nil {
				return nil, err
			}
		}
	}
...
func (p *podMountHints) findSharedMount(mount specs.Mount) *mountHint {
...
func (c *containerMounter) mountSubmounts(ctx context.Context, conf *Config, mns *fs.MountNamespace, root *fs.Dirent) error {
	for _, m := range c.mounts {
		if hint := c.hints.findSharedMount(m); hint != nil && hint.isSupported() {
